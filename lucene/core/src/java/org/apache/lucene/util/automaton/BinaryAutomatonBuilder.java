 ...
 */
public final class BinaryAutomatonBuilder {
  ...
  private BinaryAutomatonBuilder() {
    super();
  }
  ...
  /** Previous sequence added to the automaton in {@link #add(BytesRef)}. */
  private BytesRefBuilder previous  = new BytesRefBuilder();
   ...
   */
  public void add(BytesRef current) {
    ...
    assert previous == null || previous.toBytesRef().compareTo(current) <= 0
    ...
    while (pos < max && (next = state.lastChild(current.bytes[current.offset + pos] & 0xff)) != null) {
      state = next;
      pos++;
    }
  ...
  private static int convert(
    ...
    int i = 0;
    int[] labels = s.labels;
    for (BinaryAutomatonBuilder.State target : s.states) {
  ...
  public static Automaton build(Collection<BytesRef> input) {
    final BinaryAutomatonBuilder builder = new BinaryAutomatonBuilder();
   ...
   * <code>state</code>.
   */
  private void addSuffix(State state, BytesRef current, int fromIndex) {
    final int len = current.length;
    while (fromIndex < len) {
      state = state.newState(current.bytes[current.offset + fromIndex] & 0xff);
      fromIndex++;
    }
    state.is_final = true;
  }
