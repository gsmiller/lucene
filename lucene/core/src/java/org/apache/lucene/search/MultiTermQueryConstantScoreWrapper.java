 ...
 */
class MultiTermQueryConstantScoreWrapper<Q extends MultiTermQuery> extends Query
  ...
  protected static class RewritingWeight extends ConstantScoreWeight {
    ...
    protected WeightOrDocIdSetIterator rewriteInner(
      ...
      PostingsEnum reuse = backFillCollectedTerms(builder, collectedTerms, terms);
      return rewriteRemainingTerms(builder, context, fieldDocCount, termsEnum, reuse);
    }
    ...
    private WeightOrDocIdSetIterator rewrite(LeafReaderContext context, Terms terms)
      ...
      if (collectTerms(fieldDocCount, termsEnum, collectedTerms)) {
        ...
        return rewriteToBooleanQuery(collectedTerms, context);
      }
    ...
    private static boolean collectTerms(int fieldDocCount, TermsEnum termsEnum, List<TermAndState> terms)
        throws IOException {
      final int threshold =
    ...
    private WeightOrDocIdSetIterator rewriteToBooleanQuery(List<TermAndState> collectedTerms, LeafReaderContext context) throws IOException {
    ...
    private static PostingsEnum backFillCollectedTerms(DocIdSetBuilder builder, List<TermAndState> collectedTerms, Terms terms) throws IOException {
      PostingsEnum reuse = null;
      if (collectedTerms.isEmpty() == false) {
        TermsEnum termsEnum = terms.iterator();
        for (TermAndState t : collectedTerms) {
          termsEnum.seekExact(t.term, t.state);
    ...
    private WeightOrDocIdSetIterator rewriteRemainingTerms(
...
//      assert termsEnum != null;
//
//      // TODO: We should be able to do better here if we leverage an approach similar to
//      // `BooleanWeight#optionalBulkScorer`, particularly with a non-exhaustive score mode. This
//      // current approach will exhaustively populate the bitset before bulk scoring on top of it.
//      // In non-exhaustive score modes, we can early terminate, but we're not leveraging that.
//      final WeightOrDocIdSetIterator weightOrIterator;
//      final List<TermAndState> collectedTerms = new ArrayList<>();
//      if (collectTerms(fieldDocCount, termsEnum, collectedTerms)) {
//        weightOrIterator = rewriteToBooleanQuery(collectedTerms, context);
//      } else {
//        DocIdSetBuilder builder = new DocIdSetBuilder(context.reader().maxDoc(), terms);
//        PostingsEnum reuse = backFillCollectedTerms(builder, collectedTerms, terms);
//        weightOrIterator = rewriteRemainingTerms(builder, context, fieldDocCount, termsEnum, reuse);
//      }
//
//      if (weightOrIterator.weight != null) {
